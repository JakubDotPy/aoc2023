...
def solve(grid, start_pointer):
    grid.energized = collections.Counter()
    grid.add_pointers(start_pointer)
...
def starting_points(grid):
    for x in range(grid.width):
        yield support.Pointer(x, 0, support.Direction4.DOWN)
        yield support.Pointer(x, grid.width, support.Direction4.UP)
    for y in range(grid.width):
        yield support.Pointer(0, y, support.Direction4.DOWN)
        yield support.Pointer(grid.height, y, support.Direction4.UP)


def compute(s: str) -> int:
    grid = support.Grid.from_string(s)
    return max(
        solve(copy.deepcopy(grid), point)
        for point in starting_points(grid)
    )
